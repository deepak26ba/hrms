...
type LeaveRequestHandler struct {
	...
	leave   service.LeaveBalanceService
}

func NewLeaveRequestHandler(service service.LeaveRequestService, audit service.AuditService, leave service.LeaveBalanceService) *LeaveRequestHandler {
...
func (h *LeaveRequestHandler)    CreateLeaveRequest(f fiber.Ctx) error {

	var payload models.LeaveRequest
	...
	code, errorResponse := h.service.CreateLeaveRequest(payload)
...
func (h *LeaveRequestHandler) GetLeaveRequest(f fiber.Ctx) error {

	var payload, result []models.LeaveRequest
	var code int
	var count int64
	...
	if role == "ADMIN" {
		result, code, count, errorResponse = h.service.GetLeaveRequestAdmin(payload, *pageParams, *filter)
	} else {
		if currentUserID == "" {
		...
		filter.UserId = id
		result, code, count, errorResponse = h.service.GetLeaveRequest(payload, *pageParams, *filter)
	}
...
func (h *LeaveRequestHandler) GetLeaveRequestById(f fiber.Ctx) error {
	...
	result, code, errorResponse := h.service.GetLeaveRequestById(payload, filters)
	if errorResponse.Error != "" {
...
func (h *LeaveRequestHandler) PatchLeaveRequest(f fiber.Ctx) error {
	...
	result, code, errorResponse := h.service.PatchLeaveRequest(payload, id)
	if errorResponse.Error != "" {
