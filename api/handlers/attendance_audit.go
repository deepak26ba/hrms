...
type AttendanceAuditHandler struct {
	service service.AttendanceAuditService
	audit   service.AuditService
}

func NewAttendanceAuditHandler(service service.AttendanceAuditService, audit service.AuditService) *AttendanceAuditHandler {

	return &AttendanceAuditHandler{
		service: service,
		audit:   audit,
	}
...
func (h *AttendanceAuditHandler) GetAttendanceAudit(f fiber.Ctx) error {

	var payload, result []models.AttendanceAudit
	var code int
	var count int64
	...
	filter := &dto.AttendanceAuditFilter{}
	...
	if role == "ADMIN" {
		result, code, count, errorResponse = h.service.GetAttendanceAuditAdmin(payload, *pageParams, *filter)
	} else {
		if currentUserID == "" {
		...
		filter.UserId = id
		result, code, count, errorResponse = h.service.GetAttendanceAudit(payload, *pageParams, *filter)
	}
...
func (h *AttendanceAuditHandler) GetAttendanceAuditById(f fiber.Ctx) error {

	var payload models.AttendanceAudit
	...
	result, code, errorResponse := h.service.GetAttendanceAuditById(payload, filters)
	if errorResponse.Error != "" {
